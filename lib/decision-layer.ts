...
export function buildDecisionResponse(request: IntakeRequest): DecisionResponse {
  ...
  const receiptSeed = `${timestamp}:${request.source}:${playbook.id}:${input}`;
  const receiptHash = stableHash(receiptSeed);
  ...
  return {
    ...
    receipt: {
      ...
      redactionLevel: redactionLevel(request),
      signature: stableHash(`receipt:${receiptHash}:${playbook.version}`),
    },
  };
}
