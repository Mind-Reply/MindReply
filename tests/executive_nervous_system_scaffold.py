...
class ExecutiveNervousSystemTests(unittest.TestCase):
    ...
    def test_hourly_owner_report_contract_is_wired(self) -> None:
        ...
        self.assertIn("angellllkr@gmail.com", workflow)
        self.assertIn("info@mind-reply.com", workflow)
        report_shared = (ROOT / "scripts" / "report-shared.mjs").read_text(encoding="utf-8")
        report_send = (ROOT / "scripts" / "report-send.mjs").read_text(encoding="utf-8")
        report_check = (ROOT / "scripts" / "report-check.mjs").read_text(encoding="utf-8")
        self.assertIn("OWNER_EMAILS", report_shared)
        self.assertIn("OWNER_EMAILS", report_send)
        self.assertIn('"angellllkr@gmail.com"', report_check)
        self.assertIn('"info@mind-reply.com"', report_check)
    ...
    def test_premium_homepage_locale_and_search_contract(self) -> None:
        ...
        self.assertIn("Paste one messy thread", page)
        self.assertIn("See paid path", page)
        self.assertIn("Growth: repeated weekly overload", page)
        self.assertIn("Pro: team handoffs", page)
        self.assertIn("Private receipt", page)
        self.assertIn("metadataBase", layout)
        self.assertIn("Reply Overload Cleanup", layout)
